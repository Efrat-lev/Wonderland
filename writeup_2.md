...
כדי להבין איך הקלט נבדק, פתחתי את הקובץ ב-IDA. התחלתי את החיפוש בחלון ה-Strings ואיתרתי את מחרוזת הפתיחה של השלב. לחיצה כפולה הובילה אותי ל-Data Segment, ומשם השתמשתי בהפניות כדי לקפוץ אל הפונקציה שמשתמשת במחרוזת הזו - פונקציית האימות `sub_401330`.
...
הלולאה חושפת את מנגנון ההצפנה. התוכנית לא מעבדת את הקלט תו-אחר-תו. במקום זאת, ההוראה `mov ecx, dword ptr [ebp+eax+Buffer]` שולפת בלוקים של 4 בתים מתוך ה-Buffer אל האוגר `ecx`.
לאחר מכן, מופעלת הפקודה: `xor ecx, 41524241h`.

![IDA Graph View](images/l2_ida_graph.png)

 הנתונים נשמרים בזיכרון בשיטת Little Endian, הערך הקבוע `41524241h` נקרא מהסוף להתחלה כ-`41 42 52 41`. תרגום ערכי ההקסדצימל האלו לתווי ASCII חושף את מפתח ההצפנה המחזורי שלנו: **"ABRA"**.
...
**פיצוח:**
מכיוון שפעולת XOR מול אותו מפתח מבטלת את עצמה, הפעלתי סקריפט Python שלוקח את מחרוזת היעד ומבצע עליה XOR מול המפתח "ABRA" כדי לשחזר את הקלט המקורי הנדרש.
